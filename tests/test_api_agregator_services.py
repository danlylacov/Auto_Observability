...
def test_hosts_service_get_all_and_get_by_id(monkeypatch):
    ...
    # подменяем Redis-хранилище на in-memory реализацию
...
def test_hosts_service_resolve_host_for_docker(monkeypatch):
    ...
    # эмулируем отсутствие Docker окружения
    import os
...
def test_update_containers_no_hosts(monkeypatch):
    # Подготовим пустой список хостов в Redis
    ...
    # monkeypatch зависимостей внутри UpdateContainers
    monkeypatch.setattr(
    ...
    # если хостов нет, метод должен отработать без исключений и не трогать Redis
